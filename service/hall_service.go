...
func (h hallService) Add(hall entities.Hall) (entities.Hall, error) {
	err := validation.AddNewHallValidation(hall)
	if err != nil {
	...
	if err != nil {
		...
		return entities.Hall{}, errors.Wrap(errors.New(constants.FAILED_FETCHING_RESULT_FROM_DATABASE), err.Error())
	}
	if len(existingHall) > 0 {
		...
		return entities.Hall{}, errors.Wrap(errors.New(constants.HALL_CREATION_FAILED), fmt.Sprintf(constants.HALL_ALREADY_EXISTS, hall.Name, hall.TheatreId.Int64))
	}
	...
	if err != nil {
		appcontext.Logger.Error().
			...
			Msg(err.Error())
		return entities.Hall{}, errors.Wrap(errors.New(constants.ADD_REGION_FAILED), err.Error())
	}
	return hall, nil
}
...
func (h hallService) GetHallByTheatreId(theatreId int) ([]entities.Hall, error) {
	...
	if len(halls) == 0 {
		...
		return []entities.Hall{}, errors.Wrap(errors.New(constants.HALLS_DO_NOT_EXIST), fmt.Sprintf(constants.HALLS_DO_NOT_EXIST_WITH_THEATRE_ID, theatreId))
	}
	return halls, nil
}
