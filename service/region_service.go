...
func (r regionService) Add(region entities.Region) (entities.Region, error) {
	err := validation.AddNewRegionValidation(region)
	if err != nil {
	...
	if err != nil {
		...
		return entities.Region{}, errors.Wrap(errors.New(constants.FAILED_FETCHING_RESULT_FROM_DATABASE), err.Error())
	}
	if len(existingRegion) > 0 {
		...
		return entities.Region{}, errors.Wrap(errors.New(constants.REGION_CREATION_FAILED), fmt.Sprintf(constants.REGION_ALREADY_EXISTS, region.Id))
	}
	...
	if err != nil {
		appcontext.Logger.Error().
			...
			Msg(err.Error())
		return entities.Region{}, errors.Wrap(errors.New(constants.ADD_REGION_FAILED), err.Error())
	}
	return region, nil
}
...
func (r regionService) GetRegionById(regionId int) (entities.Region, error) {
	...
	if len(region) == 0 {
		...
		return entities.Region{}, errors.Wrap(errors.New(constants.REGION_CREATION_FAILED), fmt.Sprintf(constants.REGION_DOES_NOT_EXIST, regionId))
	}
	return region[0], nil
}
