...
func (s showService) Add(show entities.Show) (entities.Show, error) {
	...
	if len(hallexists) <= 0 {
		...
		return entities.Show{}, utils.WrapValidationError(errors.New(constants.FAILED_FETCHING_RESULT_FROM_DATABASE), fmt.Sprintf(constants.HALL_WITH_GIVEN_ID_DO_NOT_EXISTS, show.HallId.Int64))
	}
	...
	if err != nil {
		...
		return entities.Show{}, utils.WrapValidationError(errors.New(constants.FAILED_FETCHING_RESULT_FROM_DATABASE), err.Error())
	}
	...
	if len(existingShow) > 0 || (len(existingShow) != 0 && existingShow[0] != (entities.Show{}) && !existingShow[0].HallId.Valid) || (len(existingShow) != 0 && existingShow[0] != (entities.Show{}) && !existingShow[0].MovieId.Valid) {
		...
		return entities.Show{}, utils.WrapValidationError(errors.New(constants.REQUEST_INVALID), fmt.Sprintf(constants.SHOW_ALREADY_EXIST, show.MovieId.Int64, show.HallId.Int64, show.ShowDate))
	}
