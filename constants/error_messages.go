package constants

const (
	FAILED_STARTING_APPLICATION                     = "failed to start the application"
	FAILED_MYSQL_CONNECTION                         = "failed to connect with my sql connection"
	EMPTY_REQUEST_BODY                              = "request body is empty"
	EMAIL_ID_MANDATORY                              = "email id is missing in request"
	PASSWORD_MANDATORY                              = "password is missing in request"
	FIRST_NAME_MANDATORY                            = "first name is missing in request"
	FAILED_GET_DB_CALL                              = "failed fetching result from database"
	USER_ALREADY_EXISTS                             = "user with email id- %s already exists"
	USER_CREATION_FAILED                            = "failed to create user"
	DECODING_REQUEST_FAILED                         = "failed to decode the request body"
	USER_DOES_NOT_EXIST                             = "user does not exist"
	REGION_ID_MANDATORY                             = "region id is missing in request"
	REGION_NAME_MANDATORY                           = "region name is missing in request"
	REGION_ALREADY_EXISTS                           = "region with region id- %d already exists"
	ADD_REGION_FAILED                               = "failed to add new region"
	CREATE_BOOKING_FAILED                           = "failed to create new booking"
	ADD_MOVIE_FAILED                                = "failed to add new movie"
	REGION_TYPE_MANDATORY                           = "region type is missing in request"
	REGION_DOES_NOT_EXIST                           = "region with region id- %d do not exist"
	THEATRE_NAME_MANDATORY                          = "theatre name is missing in request"
	THEATRE_ID_MANDATORY                            = "theatre id is missing in request"
	THEATRE_ADDRESS_MANDATORY                       = "theatre address is mandatory in request"
	THEATRE_REGION_ID_MANDATORY                     = "theatre region id is missing in request"
	ADD_THEATRE_FAILED                              = "failed to add new theatre"
	ADD_SHOW_FAILED                                 = "failed to add new show"
	THEATRE_ALREADY_EXISTS                          = "theatre with details- %v already exists"
	THEATRE_DOES_NOT_EXIST                          = "theatre- %s do not exist"
	HALL_NAME_MANDATORY                             = "hall name is missing in request"
	MOVIE_ID_MANDATORY_SHOW_CREATION                = "movie id is mandatory in it's show creation request"
	HALL_ID_MANDATORY_SHOW_CREATION                 = "hall id is mandatory in it's show creation request"
	SEATS_NUMBER_INVALID                            = "number of seats are invalid in request"
	THEATRE_ID_HALL_CREATION_INVALID                = "invalid theatre-id for creating hall"
	HALL_ALREADY_EXISTS                             = "hall with hall name- %s & theatre-id %d already exists"
	MOVIE_ALREADY_EXISTS                            = "movie with name- %s already exists"
	HALLS_DO_NOT_EXIST_WITH_THEATRE_ID              = "halls with theatre id- %d do not exist"
	MOVIE_NAME_MANDATORY                            = "movie name is missing in request"
	DIRECTOR_NAME_MANDATORY                         = "director name is missing in request"
	RELEASE_DATE_MANDATORY                          = "release date is mandatory for creating movie"
	ACTIVE_MOVIES_NOT_PRESENT                       = "there are no active movies"
	SHOW_DATE_MANDATORY_IN_SHOW_CREATION            = "show date is mandatory in show creation request"
	SEAT_PRICE_MANDATORY_IN_SHOW_CREATION           = "seat price is mandatory in show creation request"
	SHOW_TIMING_DETAILS_MISSING_IN_SHOW_CREATION    = "show timing details are mandatory in show creation request"
	SHOW_START_TIME_MISSING_IN_SHOW_CREATION        = "show start time is mandatory in show creation request"
	SHOW_END_TIME_MISSING_IN_SHOW_CREATION          = "show end time is mandatory in show creation request"
	SHOW_NAME_MISSING_IN_SHOW_CREATION              = "show time name (morning/noon/evening/night) is mandatory in show creation request"
	SHOW_ALREADY_EXIST                              = "show already exist for- (movie_id:%d) , (hall_id:%d) & (show_timing:%v)"
	HALL_WITH_GIVEN_ID_DO_NOT_EXISTS                = "hall with id - %d do not exists"
	USER_ID_MANDATORY_IN_BOOKING_REQUEST            = "user-id is mandatory in booking request"
	SHOW_ID_MANDATORY_IN_BOOKING_REQUEST            = "show-id is mandatory in booking request"
	NUMBER_OF_SEATS_SHOULD_BE_VALID_BOOKING_REQUEST = "number of seats should be valid in booking request"
	SHOW_DOES_NOT_EXIST_FOR_GIVEN_SHOW_ID           = "show does not exist for given show-id"
	LESS_SEATS_AVAILABLE                            = "available seats %d, but requested seats are %d"
	BOOKING_DO_NOT_EXIST_FOR_GIVEN_USER_ID          = "booking do not exist for user-id %d"
)
