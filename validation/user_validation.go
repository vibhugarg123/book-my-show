...
func CreateNewUserRequestValidation(user entities.User) error {
	...
	if len(user.EmailId) == 0 {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.EMAIL_ID_MANDATORY)
	}
	if len(user.Password) == 0 {
		appcontext.Logger.Error().
			Str(constants.REQUEST_INVALID, constants.PASSWORD_MANDATORY).
			Msg(constants.PASSWORD_MANDATORY)
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.PASSWORD_MANDATORY)
	}
	if len(user.FirstName) == 0 {
		appcontext.Logger.Error().
			Str(constants.REQUEST_INVALID, constants.FIRST_NAME_MANDATORY).
			Msg(constants.FIRST_NAME_MANDATORY)
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.FIRST_NAME_MANDATORY)
	}
	return nil
}
