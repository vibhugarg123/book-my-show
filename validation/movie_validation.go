...
func AddNewMovieValidation(movie entities.Movie) error {
	...
	if len(movie.Name) == 0 {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.MOVIE_NAME_MANDATORY)
	}
	if len(movie.DirectorName) == 0 {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.DIRECTOR_NAME_MANDATORY)
	}
	if movie.ReleaseDate.IsZero() {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.RELEASE_DATE_MANDATORY)
	}
	return nil
}
