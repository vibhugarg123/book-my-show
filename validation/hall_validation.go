...
func AddNewHallValidation(hall entities.Hall) error {
	...
	if len(hall.Name) == 0 {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.HALL_NAME_MANDATORY)
	}
	if hall.Seats <= 0 {
		appcontext.Logger.Error().
			Str(constants.REQUEST_INVALID, constants.SEATS_NUMBER_INVALID).
			Msg(constants.SEATS_NUMBER_INVALID)
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.SEATS_NUMBER_INVALID)
	}
	if !hall.TheatreId.Valid {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.THEATRE_ID_HALL_CREATION_INVALID)
	}
	return nil
}
