...
func AddNewBookingValidation(booking entities.Booking) error {
	...
	if booking.UserId == 0 {
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.USER_ID_MANDATORY_IN_BOOKING_REQUEST)
	}
	if booking.ShowId == 0 {
		appcontext.Logger.Error().
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.SHOW_ID_MANDATORY_IN_BOOKING_REQUEST)
	}
	if booking.Seats <= 0 {
		appcontext.Logger.Error().
		...
		return errors.Wrap(errors.New(constants.REQUEST_INVALID), constants.NUMBER_OF_SEATS_SHOULD_BE_VALID_BOOKING_REQUEST)
	}
	return nil
}
